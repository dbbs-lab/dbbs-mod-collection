...
PROCEDURE rate(v(mV)) {
    ...
    TABLE s_inf,tau_s,u_inf,tau_u DEPEND Aalpha_s,Kalpha_s,V0alpha_s,Abeta_s,Kbeta_s,V0beta_s,Aalpha_u,Kalpha_u,V0alpha_u,Abeta_u,Kbeta_u,V0beta_u,celsius FROM -100 TO 30 WITH 13000
    a_s = alp_s(v)
    b_s = bet_s(v)
    a_u = alp_u(v)
    b_u = bet_u(v)
